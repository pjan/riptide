...
from riptide.cli.config import APP_PATH
from riptide.cli.utils.auth.core import load_auth_data, save_auth_data
from riptide.cli.utils.resource import TidalResource
from riptide.core.api import TidalAPI, TidalClient
from riptide.core.auth import AuthAPI
...
class ContextObject:
    ...
    def api(self):
        ...
        assert auth_data.token, "Auth Token is missing. Use `riptide auth login`"
        assert auth_data.user_id, "User ID is missing. Use `riptide auth login`"
        assert auth_data.country_code, (
            "Country Code is missing. Use `riptide auth login`"
        )

        refresh_token = auth_data.refresh_token
        assert refresh_token, "Refresh Token is missing. Use `riptide auth login`"
